...
/*   Updated: 2023/11/20 15:02:13 by navarre          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    while(s[i])
    {
        ...
        else
        {
            ...
            while(s[i + len_word] != c && s[i + len_word])
                len_word++;
            //tab[compteur] = (char *)malloc(sizeof(char) * (len_word + 1));
            //write_word(tab[compteur], s, c, i);
...
/*int    main()
{
	...
	i = 0;
	tab = ft_split("         Bienvenue Dans  La   Faille          De          L'invocateur", ' ');
    if(!tab)
    {       
